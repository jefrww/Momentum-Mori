...
func updateAnim():
	...
	get_child(2).get_child(2).set("parameters/WASDmovement/blend_position", (Vector2(1,0)*((choppedVel.length()*4)/maxSpeed)).rotated(angle).rotated(PI*1.5).reflect(Vector2(0,1)))
